...
на яку він поступив — ключ specialty, та отримані ним бали з окремих дисциплін — ключі math (математика), lang ( українська мова) та eng (англійська мова)
...
через кому без прогалин прізвище абітурієнта, код спеціальності, на яку він поступив, та отримані ним бали за окремими дисциплінами.
...
def save_applicant_data(source, output):
    with open(output, 'w') as writer:
        for element in source:
            row = f"{element.get('name')},{element.get('specialty')},{element.get('math')},{element.get('lang')},{element.get('eng')}\n"
            writer.write(row)