...
+380501112222
+380501112211
І тут ідеально підійде створення декоратора для функції sanitize_phone_number. Декоратор повинен додавати для коротких номерів префікс +38, 
...
def format_phone_number(func):
    def wrapper(phone):
        clean_phone = func(phone)
        if len(clean_phone) == 12:
            phone = f'+{clean_phone}'
        elif len(clean_phone) == 10:
            phone = f'+38{clean_phone}'     
        return phone
    return wrapper
